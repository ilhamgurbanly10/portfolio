...
// the-end-of-flash-navbar-animations



// navbar-nav

function flashCenterNavbarDropdowns() {

	const buttons = document.querySelectorAll('.fl-nav-dropdown-toggler');
	const dropdowns = document.querySelectorAll('.fl-dropdown-menu-con');

	for (let i = 0; i < dropdowns.length; i++) {
		const x = dropdowns[i].getBoundingClientRect().width / 2 + "px";
		const y = buttons[i].getBoundingClientRect().width / 2+ "px";
		dropdowns[i].style.left = "calc(-"+x+" + "+y+")";
	}
